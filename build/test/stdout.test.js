...
const Server = require('../index'),
      _require = require('../src/utils'),
      getPublicIP = _require.getPublicIP;
/**
...
describe('Initial output', () => {
  ...
  it('should print nothing straight away', () => {
    const output = stdout.inspectSync(() => {
      server = new Server(smallApp, 3001, {
        silent: true
      });
    });
  ...
  it('should print something later', () => {
    ...
    server.run().then(srv => {}).catch(console.error);
  });
  it('should print something', () => {
    ...
    let options = {
      ...
      callback(server) {
        inspect.restore(); //output[0] is the 'should print something'
  ...
  it('should signal its end', done => {
    ...
    server.run().then(srv => server.close(), err => console.log('Running error:', err)).then(closed => expect(closed).to.equal(true) && done()).catch(err => console.log('Closing error:', err));
  });
});
  ...
  it('should show its public IP', done => {
    ...
    _asyncToGenerator(function* () {
      try {
        ...
        let serv = yield server.run();
        inspect.restore(); // expect(inspect.output[inspect.output.length - 1]).to.equal(`\u001b[36mPublic IP: \u001b[35m${ip}\u001b[36m\u001b[39m\n`);
