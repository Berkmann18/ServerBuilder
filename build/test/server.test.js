...
describe('Server response', () => {
  let server = new Server((req, res) => {
    ...
    callback(srv) {
      console.log(`${srv.name} started`);
    }

  });
  const URL = 'http://localhost:3000';
  it('should run fine', done => {
    server.run().then(serv => unexpect(serv, 'to satisfy', server.server)) //expect(serv).to.deep.equal(server.server))
    .catch(err => console.error('run test error:', err)).then(done);
  });
...
describe('Attributes (HTTP)', done => {
  ...
  }, err => console.error('getPort error:', err)).catch(err => console.error('HTTPS test error:', err)).then(done);
});
...
describe('HTTP/2', done => {
  ...
  }, err => console.error('getPort error:', err)).catch(err => console.error('HTTP/2 test error:', err)).then(done);
});
describe('Wrongs', () => {
  it('should have fail on \'port\' setting', () => {
    expect(() => new Server(smallApp, 'port', {
      silent: true,
      gracefulClose: false
    }), 'port is NaN').to.throw('Port should be >= 0 and < 65536. Received NaN');
  });
  it('should have fail on negative port setting', () => {
    expect(() => new Server(smallApp, -3e3, {
      silent: true,
      gracefulClose: false
    }), 'port < 0').to.throw('Port should be >= 0 and < 65536. Received false');
  });
  it('should set bad things', () => {
    let ser = new Server(smallApp, 5e3, {
      silent: true,
      gracefulClose: false
    });
